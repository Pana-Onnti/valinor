...
function SummaryBar({ findings }: SummaryBarProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {stats.map(s => (
        <div
          key={s.label}
          className={`${s.bg} ${s.border} border rounded-2xl px-5 py-4 shadow-sm`}
        >
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-14 text-center shadow-sm">
      ...
      <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
        {filtered
          ...
          : 'No hay hallazgos activos para este cliente.'}
      </p>
      {!filtered && (
        <p className="text-xs text-gray-400 mt-1">Los hallazgos aparecerán tras ejecutar un análisis.</p>
      )}
    </div>
  )
}
        ...
        prev
          ? {
              ...
              findings: prev.findings.map(f =>
                f.id === findingId ? { ...f, status: 'false_positive' } : f
              ),
            }
          : prev
      )
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                Hallazgos · {clientId.replace(/_/g, ' ')}
              </h1>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        {/* Error banner (non-fatal, after data is loaded) */}
        {error && data && (
